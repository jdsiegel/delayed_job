...
h2. Changes in this fork (jdsiegel/delayed_job)

This fork adds a new basic @Delayed::ScheduledWorker@ class that will queue repeatable jobs at regular intervals. Scheduled jobs are specified in @$RAILS_ROOT/config/delayed_schedule.rb@:

  every 30.seconds, :run => 'Poller.refresh_data'
  every 20.minutes, :run => 'Report.update_stats'

To disable the job schedule, you can start delayed job with @script/delayed_job --disable-schedule start@.

Note that this is a simple implementation suited best for repeating tasks with small intervals. For larger intervals or more complicated cron-like recurrence rules, it is better to use cron and @script/runner@.
